...
export default function PlaylistCard({ id, title, region, cover }: { id?: string; title: string; region: string; cover: string }) {
  const href = id ? `/playlist/${id}` : '#'
  return (
    <Link href={href} className="group block overflow-hidden rounded-lg bg-white/5 ring-1 ring-white/10 hover:ring-white/20 transition">
