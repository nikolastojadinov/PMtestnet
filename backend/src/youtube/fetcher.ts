...
export async function runDiscoveryTick(arg?: { playlistIds?: string[], startRegion?: string }): Promise<TickResult> {
  ...
  for (const region of order) {
    ...
    try {
      // heartbeat
      try { await (supabase as any).from('playlists').select('id').limit(1) } catch {}

      const discovery = await runWithTimeout(() => discoverPlaylistsForRegion(region, canSpend, spend, cacheTtlDays), 60000)
      const ids = discovery.playlistIds
      if (ids.length === 0) {
        log('info', `[SUMMARY] region=${region} playlists=0 tracks=0 used_units=${unitsUsed - beforeUnits} cache_hits=${discovery.cacheHits} new_fetches=${discovery.newFetches}`)
        regionsDone++
        lastRegion = region
        await sleep(800)
      ...
      // Batch playlist metadata (playlists.list allows up to 50 IDs per req)
      ...
      log('info', `[SUMMARY] region=${region} playlists=${ids.length} tracks=${totalTracks} used_units=${used} cache_hits=${discovery.cacheHits} new_fetches=${discovery.newFetches}`)
    } catch (e: any) {
