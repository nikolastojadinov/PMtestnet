// ✅ FULL REWRITE — Jedan dnevni job u 10:15 lokalno (08:15 UTC na Renderu)
...
// 10:15 lokalno = 08:15 UTC
const SCHEDULE = '15 8 * * *';

/**
 * Fazni sistem:
 *  - Day 1 .. 29  -> FETCH (svaki dan u 10:15)
 *  - Day >= 30    -> REFRESH (svaki dan u 10:15), meta-dan = (day - 29) u intervalu 1..29 u ciklusu
 * Env:
...
export function startDailyJob() {
  cron.schedule(SCHEDULE, async () => {
    ...
    try {
      if (info.phase === 'fetch') {
        console.log(`[scheduler] 10:15 → FETCH (day=${info.day})`);
        await runFetchPlaylists({ reason: `daily-10:15-day${info.day}` });
      } else {
        console.log(`[scheduler] 10:15 → REFRESH (day=${info.day}, targetDay=${info.targetDay})`);
        await runRefreshPlaylists({ reason: `daily-10:15-day${info.day}`, targetDay: info.targetDay });
      }
    } catch (e) {
  ...
  const info = getPhaseInfo(new Date());
  console.log(`[scheduler] cron set @08:15 UTC (10:15 local); phase=${info.phase}, day=${info.day}${info.targetDay ? `, targetDay=${info.targetDay}` : ''}`);
}
