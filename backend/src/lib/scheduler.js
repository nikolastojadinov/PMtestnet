...
const TRACK_SCHEDULE    = '35 15 * * *';   // 15:35 lokalno ✅ promenjeno
...
export function startDualJobs() {
  ...
  cron.schedule(TRACK_SCHEDULE, async () => {
    try {
      console.log(`[scheduler] 15:35 (${TZ}) → Fetch Tracks`);
  ...
  console.log(`[scheduler] cron set: playlists@10:10 ${TZ}, tracks@15:35 ${TZ} (fixed only)`);
}
