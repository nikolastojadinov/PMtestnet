// ✅ FULL REWRITE — Jedan dnevni job u 10:37 lokalno (08:37 UTC na Renderu)
...
// 10:37 lokalno = 08:37 UTC
const SCHEDULE = '37 8 * * *';

/**
 * Fazni sistem:
 *  - Day 1 .. 29  -> FETCH (svaki dan u 10:37)
 *  - Day >= 30    -> REFRESH (svaki dan u 10:37), meta-dan = (day - 29) u intervalu 1..29 u ciklusu
 * Env:
...
export function startDailyJob() {
  cron.schedule(SCHEDULE, async () => {
    ...
    try {
      if (info.phase === 'fetch') {
        console.log(`[scheduler] 10:37 → FETCH (day=${info.day})`);
        await runFetchPlaylists({ reason: `daily-10:37-day${info.day}` });
      } else {
        console.log(`[scheduler] 10:37 → REFRESH (day=${info.day}, targetDay=${info.targetDay})`);
        await runRefreshPlaylists({ reason: `daily-10:37-day${info.day}`, targetDay: info.targetDay });
      }
    } catch (e) {
  ...
  const info = getPhaseInfo(new Date());
  console.log(`[scheduler] cron set @08:37 UTC (10:37 local); phase=${info.phase}, day=${info.day}${info.targetDay ? `, targetDay=${info.targetDay}` : ''}`);
}
